...
public class KotlinSourceCodeParse extends SourceCodeParse {

    public KotlinSourceCodeParse(final CodePolicy policy)
    {
        super(Language.KOTLIN, policy, policy.adv_kotlin);
        switch (policy.mode) {
            case DEFAULTS:
    ...
    private static Pattern reControl = Pattern.compile("(^|\\s+)(for|if|else(\\s+if)?|do|while|switch|when|try|catch|finally|synchronized)(\\(|\\{|\\s|$)", Pattern.MULTILINE);
    ...
    public static KotlinSourceCodeParse lex(final CodePolicy policy, final String sourceCode) {
        int n = 0;
        KotlinSourceCodeParse parse = new KotlinSourceCodeParse(policy);
        ...
        if (token.classification == Token.Classification.NOT_CLASSIFIED_YET) {
            switch (token.type) {
                case QUOT_LITERAL:
                {
                    ...
                    Matcher matcherErrorNumber = policy.getReErrorNumber_kotlin().matcher(token.source);
                    if (matcherErrorNumber.find()) {
                        ...
                        Matcher matcherPlaceholder = policy.getReErrorNumber_kotlin_placeholder().matcher(token.source);
                        if (matcherPlaceholder.matches()) {
                            token.classification = Token.Classification.PLACEHOLDER;
                            String number = matcherPlaceholder.group(policy.reErrorNumber_kotlin_placeholder_number_group);
                            if (null != number && ! "".equals(number)) {
                                ...
                                if (apiProvider.validErrorNumber(policy, errorOrdinal)) {
                                    if (globalState.store(errorOrdinal, stateItem, token)) {
                                        token.keepErrorCode = true;
                                        token.errorOrdinal = errorOrdinal;
                                        token.sourceNoErrorCode = matcherPlaceholder.group(policy.reErrorNumber_kotlin_placeholder_open_close_group) + matcherPlaceholder.group(policy.reErrorNumber_kotlin_placeholder_open_close_group);
                                    } else {
                                        token.sourceNoErrorCode = token.source = matcherPlaceholder.group(policy.reErrorNumber_kotlin_placeholder_open_close_group) + matcherPlaceholder.group(policy.reErrorNumber_kotlin_placeholder_open_close_group);
                                    }
                                } else {
                                    token.sourceNoErrorCode = token.source = matcherPlaceholder.group(policy.reErrorNumber_kotlin_placeholder_open_close_group) + matcherPlaceholder.group(policy.reErrorNumber_kotlin_placeholder_open_close_group);
                                }
                ...
                {
                    // 1) Strip '[ERR-nnnnnn] ' from string literals for re-injection
                    Matcher matcherErrorNumber = policy.getReErrorNumber_kotlin_textblocks().matcher(token.source);
                    if (matcherErrorNumber.find()) {
                        token.classification = Token.Classification.ERROR_NUMBER;
