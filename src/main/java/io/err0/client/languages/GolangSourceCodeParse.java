...
public class GolangSourceCodeParse extends SourceCodeParse {
    ...
    public GolangSourceCodeParse(final CodePolicy policy) {
        ...
        switch (policy.mode) {
            case DEFAULTS:
                reLogger = Pattern.compile("(^|\\s+)(m?_?)*log(ger)?\\.(crit(ical)?|log|fatal|err(or)?|warn(ing)?|info|fault|notice)f?\\s*\\(\\s*$", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
                break;
            ...
            switch (token.type) {
                ...
                case QUOT_LITERAL:
                {
                    ...
                    if (matcherErrorNumber.find()) {
                        ...
                        Matcher matcherPlaceholder = policy.getReErrorNumber_go_placeholder().matcher(token.source);
                        if (matcherPlaceholder.matches()) {
                            token.classification = Token.Classification.PLACEHOLDER;
                            String number = matcherPlaceholder.group(policy.reErrorNumber_go_placeholder_number_group);
                            if (null != number && ! "".equals(number)) {
                                ...
                                if (apiProvider.validErrorNumber(policy, errorOrdinal)) {
                                    if (globalState.store(errorOrdinal, stateItem, token)) {
                                        token.keepErrorCode = true;
                                        token.errorOrdinal = errorOrdinal;
                                        token.sourceNoErrorCode = matcherPlaceholder.group(policy.reErrorNumber_go_placeholder_open_close_group) + matcherPlaceholder.group(policy.reErrorNumber_go_placeholder_open_close_group);
                                    } else {
                                        token.sourceNoErrorCode = token.source = matcherPlaceholder.group(policy.reErrorNumber_go_placeholder_open_close_group) + matcherPlaceholder.group(policy.reErrorNumber_go_placeholder_open_close_group);
                                    }
                                } else {
                                    token.sourceNoErrorCode = token.source = matcherPlaceholder.group(policy.reErrorNumber_go_placeholder_open_close_group) + matcherPlaceholder.group(policy.reErrorNumber_go_placeholder_open_close_group);
                                }
