...
def exception_examples
  ...
  raise ExceptionClass, '[E-1] Example one, single quoted strings.'
  raise ExceptionClass, "[E-2] Example two, double quoted strings."
  raise ExceptionClass,
    "[E-3] Example three, string on another line." \
    "with continuation on a line afterwards."
  raise ExceptionClass, <<HEREDOC
[E-4] Example four, heredoc type 1, double quote string.
HEREDOC
  raise ExceptionClass, <<-HEREDOC
[E-5] Example four, heredoc type 2, indented.
  HEREDOC
  raise ExceptionClass, <<~"HEREDOC"
    [E-6] Example five, heredoc type 3, squiggly.
  HEREDOC
  raise ExceptionClass, <<'HEREDOC'
[E-7] Example six, heredoc type 4, single quoted string.
HEREDOC
  raise ExceptionClass, <<-'HEREDOC'
[E-8] Example seven, heredoc type 5, single quoted string, indented.
  HEREDOC
  raise ExceptionClass, <<~'HEREDOC'
    [E-9] Example eight, heredoc type 6, single quoted string, squiggly.
  HEREDOC

end

def syntax_examples
  raise IOError, "[E-10] closed stream" if closed?
end