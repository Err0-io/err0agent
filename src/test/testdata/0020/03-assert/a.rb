# from ruby on rails - MIT License
  ...
  module Routing
    ...
    class RouteSet
      ...
      def add_route(mapping, name)
        raise ArgumentError, "[E-1] Invalid route name: '#{name}'" unless name.blank? || name.to_s.match(/^[_a-z]\w*$/i)

        if name && named_routes[name]
          raise ArgumentError, "[E-2] Invalid route name, already in use: '#{name}' \n" \
      ...
      def recognize_path_with_request(req, path, extras, raise_on_missing: true)
        @router.recognize(req) do |route, params|
          ...
          if app.matches?(req) && app.dispatcher?
            begin
              ...
              raise ActionController::RoutingError, "[E-3] A route matches #{path.inspect}, but references missing controller: #{params[:controller].camelize}Controller"
            end
        ...
        if raise_on_missing
          raise ActionController::RoutingError, "[E-4] No route matches #{path.inspect}"
        end
      end
    end
    # :startdoc:
